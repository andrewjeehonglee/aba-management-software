...
function ClientTeamRow({ row }: { row: RosterRow }) {
  ...
  return (
    <Link
      ...
      className={`${TEAM_GRID} rounded-[14px] border border-line/80 bg-surface px-4 py-3.5 transition-colors hover:border-brand/25 hover:bg-surface-2`}
    >
  ...
  return (
    <section className="w-fit max-w-full">
      <div className="mb-4 flex flex-wrap items-baseline gap-x-3 gap-y-1">
        ...
        <span className="text-[15px] font-semibold uppercase tracking-[0.08em] text-muted">
          BCBA
        </span>
    ...
    <OwnerAppShell ownerName={ownerName} practiceName={practiceName}>
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        <div className="owner-scroll min-h-0 flex-1 overflow-y-auto pr-1">
          {loading && (
            <p className="py-16 text-center text-[16px] text-muted animate-pulse">Loading clients…</p>
          )}

          {error && (
            <div className="rounded-[var(--radius)] bg-surface p-8 text-center shadow-card">
              <p className="text-[16px] text-alert">Could not load clients. Please refresh and try again.</p>
            </div>
          )}
